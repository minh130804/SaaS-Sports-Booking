...
const AdminDashboard = () => {
    ...
    const fetchRevenue = async (selectedYear) => {
        setLoading(true);
        try {
            const res = await axiosClient.get(`/statistics/admin-revenue?year=${selectedYear}`);
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <Title level={2} style={{ color: '#0f172a' }}>Thống kê Doanh thu Gói cước (Admin)</Title>
                <Select 
                    value={year} 
            ...
            <Row gutter={16} style={{ marginBottom: 24 }}>
                <Col span={12}>
                    <Card style={{ borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', border: 'none' }}>
                        <Statistic
                            title={<span style={{ fontSize: 16, color: '#64748b' }}>Tổng doanh thu bán gói cước năm {year}</span>}
                            value={totalRevenue}
                            formatter={formatCurrency}
                            valueStyle={{ color: '#8b5cf6', fontWeight: 800, fontSize: 32 }}
                ...
                <Col span={12}>
                    <Card style={{ borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', border: 'none' }}>
                        <Statistic
                            ...
                            formatter={formatCurrency}
                            valueStyle={{ color: '#ec4899', fontWeight: 800, fontSize: 32 }}
                            prefix={<RiseOutlined />}
                        />
                    </Card>
                </Col>
            ...
            <Card style={{ borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', border: 'none' }}>
                <Title level={4} style={{ marginBottom: 24, color: '#334155' }}>Biểu đồ tăng trưởng doanh thu Gói cước</Title>
                    ...
                    <div style={{ width: '100%', height: 400 }}>
                        <ResponsiveContainer>
                            ...
                            >
                                ...
                                <Area type="monotone" dataKey="revenue" name="Doanh thu gói cước" stroke="#8b5cf6" strokeWidth={3} fillOpacity={1} fill="url(#colorAdmin)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                )}
